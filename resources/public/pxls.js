	...
	const ls = storageFactory(localStorage, "ls_", 99),
		...
		ban = (function() {
			const self = {
				...
				init: function() {
					...
					const custom_evt = window.CustomEvent;
					...
					const evt_old = window.document.createEvent;
		...
		info = (function() {
			const self = {
				init: function() {
					...
					let auto_reset = ls.get("auto_reset");
					if (auto_reset === null) {
						auto_reset = true;
					}
		...
		notification = (function() {
			const self = {
				...
				show: function(s) {
					try {
						const n = new Notification("pxls.space", {
							body: s,
							icon: "favicon.ico",
						});
						n.onclick = function() {
